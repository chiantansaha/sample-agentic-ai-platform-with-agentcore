# Backend Infrastructure Modules for Dev Environment
...
module "dynamodb" {
  ...
  enable_pitr                = false # Dev: PITR 비활성화
  enable_deletion_protection = false # Dev: 삭제 보호 비활성화
}
...
module "iam" {
  ...
  sqs_sync_queue_arn             = "arn:aws:sqs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:${var.project_name}-kb-sync-checker-queue-${var.environment}"
...
module "sqs_lambda" {
  ...
  log_retention_days             = 365 # Dev: 365일
}
