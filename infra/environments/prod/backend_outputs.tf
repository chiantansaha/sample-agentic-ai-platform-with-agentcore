...
output "dynamodb_team_tag_table_name" {
  description = "TeamTagManagement table name"
  value       = module.dynamodb.team_tag_management_table_name
}
...
output "dynamodb_api_catalog_table_name" {
  description = "DynamoDB API Catalog table name"
  value       = aws_dynamodb_table.api_catalog.name
}
...
output "cognito_oauth_token_url" {
  ...
  value       = "https://${var.project_name}-mcp.auth.${var.aws_region}.amazoncognito.com/oauth2/token"
}
