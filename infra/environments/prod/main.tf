...
resource "aws_vpc" "main" {
  cidr_block           = "10.1.0.0/16"  # Prod는 다른 CIDR 사용
...
resource "aws_security_group" "alb" {
  ...
  #checkov:skip=CKV_AWS_260:ALB requires public HTTP access for redirect to HTTPS
  ingress {
...
# Route53 - Reference existing hosted zone (managed by dev)
data "aws_route53_zone" "main" {
  name = "agentcore.ai.kr"
}

# Route53 A record for prod (root domain)
...
resource "aws_ecs_service" "main" {
  ...
  desired_count   = 2  # 내부 서비스용으로 적정 수준
  launch_type     = "FARGATE"
  ...
  container_definitions = jsonencode([
    ...
    },
    {
      ...
      environment = [
        ...
        { name = "DYNAMODB_TEAMTAG_TABLE", value = module.dynamodb.team_tag_management_table_name },
        ...
        { name = "DYNAMODB_API_CATALOG_TABLE", value = aws_dynamodb_table.api_catalog.name },
        # S3 Buckets
        ...
        # ECR
        { name = "PLAYGROUND_ECR_REPOSITORY", value = module.ecr.playground_agents_repository_url },
        { name = "BASE_IMAGE_URI", value = module.base_image_builder.base_image_uri },
        # SQS
        { name = "SQS_KB_CREATION_QUEUE_URL", value = module.lambda.kb_creation_queue_url }
      ]
...
resource "aws_cloudwatch_log_group" "main" {
  ...
  retention_in_days = 365  # Prod는 더 긴 로그 보존
...
# ============================================================
# IAM Role Policy for Backend Playground
# ============================================================

resource "aws_iam_role_policy" "backend_playground" {
  name = "${var.project_name}-backend-playground-policy"
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          ...
          module.dynamodb.team_tag_management_table_arn,
          "${module.dynamodb.team_tag_management_table_arn}/index/*",
          ...
          aws_dynamodb_table.api_catalog.arn,
          "${aws_dynamodb_table.api_catalog.arn}/index/*"
        ]
      },
      {
        ...
        Resource = "arn:aws:sqs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:${var.project_name}-kb-creation-queue"
      },
      {
...
resource "aws_cognito_user_pool" "mcp" {
  name = "${var.project_name}-mcp-pool"
...
resource "aws_cognito_user_pool_domain" "mcp" {
  domain       = "${var.project_name}-mcp-${data.aws_caller_identity.current.account_id}-${var.aws_region}"
...
# =============================================================================
# terraform apply 시 apps/backend/.env.production 파일 자동 생성
# 이 파일은 로컬에서 프로덕션 리소스에 접근할 때 사용 (주의 필요)

resource "local_file" "backend_env_production" {
  filename = "${path.module}/../../../apps/backend/.env.production"
  content  = <<-EOT
# Auto-generated by Terraform (${var.environment} environment)
# Generated at: ${timestamp()}
# DO NOT EDIT MANUALLY - This file is managed by Terraform
#
# WARNING: 이 파일은 프로덕션 리소스에 접근합니다. 로컬에서 사용 시 주의하세요!
# 프로덕션(ECS)에서는 Terraform이 설정한 환경변수가 사용됩니다.
...
VERSION=0.1.0
CORS_ORIGINS=https://agentcore.ai.kr
...
DYNAMODB_TEAMTAG_TABLE=${module.dynamodb.team_tag_management_table_name}
...
DYNAMODB_API_CATALOG_TABLE=${aws_dynamodb_table.api_catalog.name}
...
PLAYGROUND_ECR_REPOSITORY=${module.ecr.playground_agents_repository_url}
...
# SQS (from Terraform)
# =====================
SQS_KB_CREATION_QUEUE_URL=${module.lambda.kb_creation_queue_url}
